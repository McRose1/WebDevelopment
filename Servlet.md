...
这种结构是将需要处理的业务合理地分配到客户端和服务器端，这样可以大大降低通信成本，但是升级维护相对困难。

手机中的微信、QQ、王者荣耀等应用程序就是 C/S 架构。

## B/S：浏览器/服务器(Browser/Server)
B/S 结构是随着互联网的发展，web 出现后兴起的一种网络结构模式。这种模式统一了客户端，让核心的业务处理在服务端完成。

只要在电脑和手机上安装一个浏览器，就可以通过 web server 与数据库进行数据交互，比如在手机或电脑上用浏览器上百度搜索、看新闻等就是使用 B/S 结构进行数据交互。

1. 用户操作客户端，即对页面进行操作
2. 客户端页面获取用户操作需要的信息，比如用户登录时的用户名密码，或对页面进行了点击，需要异步加载数据，将需要发送到服务器进行处理的数据获取下来
3. 通过 HTTP 的通信模块对 2 中获取的信息进行打包
4. 通过 ajax 等技术将数据发送到服务端
5. 在服务端通过 HTTP 将传过来的包进行拆包
6. Servlet 通过 request 获取客户端发来的请求
7. 在 dao 层，即业务逻辑层对获取的请求进行处理，然后与数据库进行交互
8. 将 dao 层得出的处理结果返回到 Servlet，通过 response 返回客户端
9. 对要返回的响应打包，通过 HTTP 协议发送到客户端
10. 将 HTTP 传过来的包进行拆包
11. 对响应进行解析（传过来的时 json 或其他东西），通过浏览器体现出来

### B/S vs. C/S
![B/S&C/S](/images/B:S%26C:S.png)

不同点：
1. 硬件环境不同：C/S 通常是建立在专用的网络上，小范围的网络环境；而 B/S 是建立在广域网上的，适应范围强，通常由操作系统和浏览器就行
2. C/S 结构比 B/S 结构更安全，因为用户群相对固定，对信息的保护更强
3. B/S 结构维护升级比较简单；而 C/S 结构维护升级相对困难
4. 传统的 C/S 架构需要单独安装客户端，一但客户端版本发送变化，需要更新；而 B/S 架构客户端使用的是浏览器
5. B/S 开发较 C/S 相对简单，C/S 模式需要我们在客户端和服务端编写通信模块和自定义的通信协议；但 B/S 结构则使用 HTTP 协议
